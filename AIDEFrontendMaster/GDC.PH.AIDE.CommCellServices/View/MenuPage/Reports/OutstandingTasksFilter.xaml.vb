...
Public Class OutstandingTasksFilter
...
#Region "Events"
    Private Sub btnUpdate_Click(sender As Object, e As RoutedEventArgs) Handles btnUpdate.Click
        Try
            ...
            'Process.Start(reportsModel.FILE_PATH, CStr(empId))
            Dim result As Integer
            Dim dateStart As String
            'Dim empIdstr As String
            ...
            '    empIdstr = CStr(empId)
            'End If
            If Not dateStart1.SelectedDate Is Nothing Then
                dateStart = CStr(CDate(dateStart1.SelectedDate).Month) + "-" + CStr(CDate(dateStart1.SelectedDate).Day) + "-" + CStr(CDate(dateStart1.SelectedDate).Year)
            Else
            ...
            Else
                processStartInfo.Arguments = CStr(empId) & " " & dateStart
            End If
            ...
            If output.Contains("Report generated") Then 'Replace text with the text that shows it was successful in the command prompt
                ...
                If result = 6 Then
                    Process.Start("C:\GeneratedReports\Outstanding Task Report.xlsx")
                End If
            Else
                'blocker_failed.Show()
            ...
            'result = MsgBox("Report has been downloaded. Open the Report Now?", MessageBoxButton.YesNo, "AIDE")
            'If result = 6 Then
            '    Process.Start("E:\GeneratedReports\Outstanding Task Report.xlsx")
            'End If
            ...
            '            Process.Start(reportsModel.FILE_PATH & empId)
...
#Region "Functions"
    ...
    Public Sub ListOfAllUser()
        ...
        Dim lstAllUserInfo As New NicknameModel
        lstAllUserInfo.EMP_ID = 0
        lstAllUserInfo.EMPLOYEE_NAME = "ALL"
        lstAllUserInfo.First_Name = "ALL"
        lstAllUserInfo.Nick_Name = "ALL"

        lstNicknameList.Add(lstAllUserInfo)

        For Each rawUser As MyNickname In successRegisterDBProvider.GetMyNickname()
            lstNicknameList.Add(New NicknameModel(rawUser))
            If allEmpId = "" Then
                allEmpId = CStr(rawUser.EmpID)
            Else
                allEmpId = allEmpId + """" + "," + """" & CStr(rawUser.EmpID)
            End If
        Next
        ...
        'cbNickname.Items.Add(New With {.Text = "All", .Value = 0})
    End Sub
